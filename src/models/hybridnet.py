...
class HybridNet(nn.Module):
    def __init__(self, num_classes=2, dropout=0.0):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=num_channels,
                      kernel_size=(1, 61), stride=(1, 1), padding=0),
            nn.LeakyReLU())
        ...
        self.layer3 = nn.Sequential(
            nn.Dropout(p=dropout),
            nn.Conv2d(in_channels=num_channels, out_channels=num_channels *
                      2, kernel_size=(1, 31), stride=(1, 1)),
            nn.MaxPool2d(kernel_size=(1, 2), stride=(1, 2)),
            nn.LeakyReLU())
        
        self.layer4 = nn.Sequential(
            nn.BatchNorm2d(num_channels*2),
            nn.Conv2d(in_channels=num_channels*2, out_channels=num_channels *
                      4, kernel_size=(1, 21), stride=(1, 1)),
            nn.LeakyReLU())
        
        self.layer5 = nn.Sequential(
            nn.Dropout(p=dropout),
            nn.Conv2d(in_channels=num_channels*4, out_channels=num_channels *
                      1, kernel_size=(7, 1), stride=(7, 1)),
            nn.LeakyReLU())
    ...
    def forward(self, x):
        ...
        out = self.decLayer1(inter.unsqueeze(1).type(
            torch.cuda.FloatTensor).unsqueeze(1).type(torch.cuda.FloatTensor))
